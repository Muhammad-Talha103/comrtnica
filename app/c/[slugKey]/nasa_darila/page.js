  ...
  return (
    <CompanyAccountLayout isMobileViewChange={true}>
      <div className="w-full mobile:max-w-[500px] mobile:mx-auto max-w-[969px]">
        ...
        <div className="w-full mobile:mt-0 mt-[30px] flex justify-start">
          <div className="w-full">
            <div className="tabletUserAcc:block hidden text-[#717B8C] text-[14px] leading-[19px] mt-[60px] mb-[12px]">
            ...
            <table className="max-w-full mobile:w-full w-[984px] border-separate border-spacing-y-[0px] mobileUserAcc:mt-[20px] mobileUserAcc:border-spacing-y-[0px] tabletUserAcc:border-spacing-y-[0px] border-spacing-x-[0px] mt-[42px]">
              <thead className="text-[14px] leading-[24px] font-variation-customOpt14 text-[#3C3E41] uppercase">
                <tr className="leading-[14px]">
                  <th className="mobile:p-0 p-[14px] text-left mobile:pt-[14px] mobile:w-[2%] w-[16%]">
                    <span className="mobile:hidden block">dan</span>
                  </th>
                  <th className="mobile:pl-0 p-[14px] mobile:text-[#6D778E] mobile:font-semibold text-left w-[25%] tabletUserAcc:w-[30%] mobileUserAcc:w-[30%]">
                    spominska
                  </th>
                  <th className="p-[14px] mobile:hidden text-left w-[18%]">
                    mesto
                  </th>
                  <th className="p-[14px] mobile:hidden text-left w-[15%]">
                    komu
                  </th>
                  <th className="mobile:px-2 p-[14px] mobile:text-[#6D778E] mobile:font-semibold text-left mobile:w-[5%] w-[30%]">
                    kaj
                  </th>
                </tr>
              </thead>
              <tbody>
                {logs.map((log, index) => {
                  ...
                  const rowBg = isEven
                    ? "mobile:bg-white/40 bg-white/40"
                    : "mobile:bg-white/40 bg-[#e3e9f0]";
                  const keeperTime = getKeeperTypeById(log.time);

                  return (
                    <tr key={index} className={`${rowBg} text-[#3C3E41]`}>
                      <td className="mobile:px-2 px-4 py-[18px] text-[16px] leading-[19px] font-variation-customOpt18 mobile:rounded-0 mobile:border-l-0 border-t border-b border-l mobile:rounded-[0px] rounded-l border-[#A1B1D4] text-[#6D778E]">
                      ...
                      <td className="mobile:pl-0 px-4 py-[12px] border-t border-b border-[#A1B1D4] text-center text-[16px]">
                        <div className="flex flex-col items-start gap-0">
                          <span className="text-[#6D778E] mobile:text-[14px] text-[12px]">
                            {log.name}
                          </span>
                          <span className="text-[#3C3E41] mobile:text-[16px] text-[14px]">
                            {log.surname}
                          </span>
                        </div>
                      </td>
                      <td className="p-4 mobile:hidden leading-[19px] font-variation-customOpt16 border-t border-b border-[#A1B1D4] text-[14px]">
                        {log.city}
                      </td>
                      <td className="p-4 mobile:hidden leading-[19px] font-variation-customOpt16 border-t border-b border-[#A1B1D4] text-[14px]">
                        {log.giftedTo}
                      </td>
                      <td className="mobile:px-2 p-4 leading-[19px] font-variation-customOpt16 mobile:rounded-[0px] mobile:border-l-0 mobile:border-t mobile:border-b mobile:border-r-0 border-t border-b border-r rounded-r border-[#A1B1D4] text-[14px]">
